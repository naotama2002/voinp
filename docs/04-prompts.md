...
|---|---|---|
| 7 | **empty** | 修復後に空 |
| 8 | **lengthRatio** | `r = candidate.count / raw.count` (書記素数)。**2 つの基準の和集合**で、どちらかを満たせば通す: (a) `policy.lengthRatio` に `r` が収まる、(b) `raw.count < 20` かつ `abs(差) ≤ absoluteSlackForShortInput`。(b) は短文専用の救済 (短文では比がただのノイズになる)。要約 (`r` 小) と「質問に答えた」(`r` 大) を捕まえる |
...
| 指示に含まれる語 | 緩める検査 |
|---|---|
| 英訳 / translate / 英語に | 文字種・内容保持・疑問文の形・長さ比 (`0.3...3.0`) |
| 短く / 簡潔 / 要約 / まとめ | 長さ比の下限 |
...
長さ判定はかつて「20 文字未満なら絶対差だけを見る」という**排他分岐**だった。
そのため `policy.lengthRatio` をいくら緩めても短文には届かず、
「英訳して」で 15 文字の日本語を訳した 41 文字の英語が、
比率 2.7 は許容範囲 (`0.3...3.0`) に収まっているのに
絶対差 26 > slack 15 で棄却されていた。
日本語→英語は文字数が素直に 2〜3 倍になるので、**短い発話ほど必ずこれに当たる**。

→ 2 つの基準は**和集合**にする。絶対差は短文専用の救済であって、
比率判定を置き換えるものではない。`GuardThresholdTests` で両方向を固定している
（短い英訳が通ること / 緩和なしの短文の暴走は依然として棄却されること）。
