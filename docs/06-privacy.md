...
## クラウド STT を入れた（実装済み・オプトイン）

**旗印の文言は変えていない。** 「デフォルト起動状態では」という限定句が、
下の 5 条件で機械的に定義されているため。

### 旗印を維持する唯一の分岐点

`Settings.cloudTranscriptionDestination` が音声の送信先を返す唯一の場所で、
**5 条件の論理積**になっている。

1. `privacy.allowNetwork`
2. `transcription.provider == "openai.realtime"`
3. 接続先が書かれていてホストが取れる
4. **そのホストに対して**同意している
5. 同意したときの文面が現行版（`AudioEgressNotice.currentVersion`）

`EgressPolicySnapshot.derive` はこれが非 nil のときだけ `.transcribe` を
許可集合に足す。**リテラルで書かない。** `verify-privacy.sh` の検査 7 が
grep で単一性を見ている。

同意は**ホスト単位**なので、接続先を変えると自動的に失効する。
文面の版を上げると全員の同意が失効し、次に使うときに再確認が出る。

同意は `UserDefaults` ではなく `config.json` の `privacy.audioEgress` に置く。
理由は 2 つ:
- `derive` と `evaluate` が「設定だけの純粋関数」である性質を壊さない
  （I/O が混ざるとテストできなくなる）
- 同僚やセキュリティ担当が、設定ファイルを見て同意の事実を確かめられる

> **正直に書く留保**: `config.json` を手で書けば同意も書ける。
> ただしこの章の脅威モデルは「守る相手は悪意ある攻撃者ではなく、自分たちの不注意」と
> 宣言しているので、そこは守備範囲外である。

### ゴールデンテスト

`AudioEgressGoldenTests` が 5 条件の**全組合せ 32 通り**を回し、
全部揃ったときだけ送信先が現れることを確かめる。
条件を足し忘れる、論理積を論理和に書き間違える、といった事故を拾う。

わざと壊して落ちることを確認してある:
- 同意の検査を外す → 5 件が落ち、`mask=23 consent=false` を名指しする
- `derive` に `.transcribe` を直書き → 2 件が落ちる

`config.example.jsonc` が既定で音声を出さないことも、**実際のパーサで読んで**検査する
（シェルで JSON を切り出そうとすると `https://` の `//` をコメントとして消してしまう。
実際に書いて踏んだ）。
...
| 状態 | メニューバー |
|---|---|
| 権限不足 | `exclamationmark.triangle` |
| **音声が外へ出る** | `antenna.radiowaves.left.and.right` |
| テキストのみ外へ | `globe` |
| 完全ローカル | `mic` |

到達範囲が同じでも**データ種別が音声なら厳しい側へ振る**。これは
「申告でアイコンを優しくしない」原則の裏返しで、緩める方向には一切使わない。

HUD には常にどちらのエンジンかを出す。**「クラウドのときだけ出す」にしない。**
印が無いことは情報にならず、見落としと区別できないため、ローカルのときも肯定形で書く。

退避したときは「ローカルに退避」と出す。**黙って切り替えない。**
...
`VoinpNet` を別プロセスに切り出して音声プロセスからネットワーク権限を剥奪する案は、
**実装していない**。`EgressRequest` / `EgressResponse` が `Codable` の値型のままなので
移行の余地は残っているが、いまは同一プロセスで動く。
代わりに、`voinp-offline` にネットワーク実装が 1 バイトも入っていないことを
`nm` で検査している（対照群つき）。
