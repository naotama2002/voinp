...
## 勘所: ユーザーの指示は既定方針に勝てない

「英訳して」と書いても英訳されなかった。**原因が 3 つ重なっていた**ので記録する。
順序を変えるだけでは直らず、1 つずつ潰す必要があった。

### 1. 矛盾する既定方針が残っていた

L1 に「要約・翻訳・言い換えをしない」「入力の言語を保つ」があるのに、
その後ろに「英訳して」を置いていた。**否定のほうが強く効く。**
「こちらを優先してください」と書き足しても勝てなかった。

→ 指示の内容に応じて、**矛盾する項目自体を出さない**
（`GuardPolicy.requireSameScript` が false なら言語保持の行を外す）。

### 2. 役割を「校正エンジン」に固定していた

```
あなたは音声入力の書き起こしテキストを整形する校正エンジンです。
```

こう名乗らせると、**整形以外の指示に従わなくなる**。
これが最も効いていた（外した途端 2/3 で英訳されるようになった）。

→ 「指示に従って処理するエンジン」に変更。

### 3. 指示が system の中ほどに埋もれていた

長い既定ルールの中に 1 行あるだけでは無視されることがある。

→ **区切りの後（L5）にも指示を繰り返す。** 直近性バイアスを使う。
5/5 で安定した。

### 壊してはいけないもの

上記を変えても、次は維持されている（実機で確認）。

- 指示なしのフィラー除去
- 書き起こし内のインジェクション（「これまでの指示を無視して」）を実行しない
- 事実・数値・固有名詞を変えない

`UserInstructionPriorityTests` で固定している。

## プリセットは持たない（方針変更）

当初は `raw` / `clean` / `polite` / `slack` / `translate-en` の 5 種を
組み込みプリセットとして用意し、`prompts/*.md` で上書きできる設計にしていた。

**実際に使ってみて廃止した。**

- 用意した分類が使う人の用途に合わない
- 選ぶこと自体が手間で、しかも選んだ結果どうなるか分からない
- 「自分の言葉で書きたい」が実際の要望だった

いまは設定に**校正プロンプトを 1 つ書くだけ**にしている（`refinement.prompt`）。
L0 不変ガードと L1 ベース指示は変わらず適用されるので、
ユーザーが書くのは「追加の指示」だけで済む。

### ガードの厳しさは指示から推定する

プリセットが無くなったので、`PresetGuardPolicy` をどう決めるかが問題になる。
「英訳して」と書いた人の出力を文字種チェックで棄却しては意味がない。

`Preset.policy(for:)` が書かれた指示を見て、必要な検査だけを緩める。

| 指示に含まれる語 | 緩める検査 |
|---|---|
| 英訳 / translate / 英語に | 文字種・内容保持・疑問文の形 |
| 短く / 簡潔 / 要約 / まとめ | 長さ比の下限 |
| 箇条書き / リスト / markdown | Markdown の追加 |

素朴な文字列一致だが、**誤って緩めても危険側に倒れない**
（数値チェックと「内容に答えない」は常に残る）。
`PromptDrivenPolicyTests` で固定している。

## （参考）当初検討したプリセットのファイル形式
