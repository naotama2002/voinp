...
### 勘所: バイナリを直接起動すると TCC の許可がターミナルに付く

**`make run` は `open -n` を使う。バイナリを直接実行してはいけない。**

```sh
# 駄目な例
~/Applications/Voinp.app/Contents/MacOS/voinp
```

こうするとプロセスはシェルの子になり(実測で親が `/bin/zsh` であることを確認)、
TCC は**責任プロセス**をターミナルアプリと判定する。
この状態でアクセシビリティを要求すると、システム設定に現れて許可されるのは
**Ghostty / Terminal.app のほう**で、Voinp はいつまで経っても権限を得られない。

LaunchServices 経由 (`open -n`) で起動すれば、アプリ自身が責任プロセスになり、
許可は正しく Voinp に紐づく。

代償として stdout がターミナルに出なくなるので、ログは unified log から拾う:

```sh
make logs          # log stream --predicate 'subsystem == "com.naotama2002.voinp"'
make logs-recent   # 直近 10 分
```

権限を必要としない部分だけを見たいときのために `make run-unattached` を残してあるが、
警告を出したうえで直接起動する形にしてある。

**もしターミナルアプリを誤って許可してしまったら**、
システム設定 → プライバシーとセキュリティ → アクセシビリティ から
そのターミナルのチェックを外すこと。付けたままだと、
そのターミナルから起動した任意のプロセスが同じ権限で動けてしまう。

> 署名が正しくても起動経路を誤ると許可先がずれる。
> designated requirement (上記) と責任プロセスは**別の問題**である。
